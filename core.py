"""
core.py â€” Prediction Engine (V2)
----------------------------------
...
from config import FEATURES, HORIZONS, METRICS_PATH, MODEL_PATH, N_TIMESTEPS, SCALER_PATH
from features import engineer_features, fetch_data
from model import TemporalAttention


def load_artifacts():
    """Load the trained model, scaler, and optional threshold metadata."""
    if not (os.path.exists(MODEL_PATH) and os.path.exists(SCALER_PATH)):
        raise FileNotFoundError("Model artifacts not found. Run: python train.py")
...
class ModelEngine:
    ...
    def _load(self):
        try:
            self.model, self.scaler, self.thresholds = load_artifacts()
            self.ready = True
            self.last_error = None
    ...
    def predict(self, symbol):
        ...
        try:
            ...
            for idx, horizon in enumerate(HORIZONS.keys()):
                ...
                thresh = self.thresholds.get(horizon, 0.5)

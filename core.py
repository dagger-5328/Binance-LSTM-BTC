...
from config import COINS, FEATURES, HORIZONS, N_TIMESTEPS, MODEL_PATH, SCALER_PATH, METRICS_PATH
...
class ModelEngine:
    ...
    def predict(self, symbol):
        ...
        if not self.ready:
            return {"error": "Model not trained. Run: python train.py"}
        ...
        for idx, h in enumerate(horizons):
            ...
            # Use the optimized threshold to make the directional decision
            direction = "UP" if prob > thresh else "DOWN"
            
            # Re-scale confidence so that the threshold is represented as 50%
            if prob > thresh:
